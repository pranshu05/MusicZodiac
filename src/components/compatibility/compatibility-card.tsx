...
export const CompatibilityCard = ({ user, score, matchingSigns, matchingArtists, matchingElements }: { user: any; score: number; matchingSigns: string[]; matchingArtists: number; matchingElements: number }) => (
    <Link href={`/profile/${user.username}`} className="block">
        <div className="flex items-center justify-between p-2 sm:p-4 bg-purple-900/30 rounded-lg border border-purple-500/20 hover:border-purple-500/40 transition-all">
            ...
            <div className="text-right">
                <div className="text-xl sm:text-2xl font-bold text-pink-400 text-glow-pink">{score}%</div>
                <div className="w-24 h-2 bg-purple-900/50 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-pink-500 to-purple-500 rounded-full" style={{ width: `${score}%` }}></div>
                </div>
            </div>
        </div>
    </Link>
